...
if st.button('Submit', key='submit_btn'):
    if time_series_interval == 'Daily':
        ...
        url = f'https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY&symbol={symbol_input}&apikey=5EJNPQV0T263M9MG'
        response = requests.get(url)
        if response.status_code == 200:
            data = response.json()
        ...
        url = f'https://www.alphavantage.co/query?function=TIME_SERIES_MONTHLY&symbol={symbol_input}&apikey=5EJNPQV0T263M9MG'
        response = requests.get(url)
        if response.status_code == 200:
            data = response.json()
